...
namespace StateForge
{
    ...
    partial class CoderContext : CoderBase
    {
        ...
        private void WriteClass(StateType state)
        {
            ...
            if (state.Type.HasFlag(StateType.TypeFlags.TOP) == true)
            {
                ...
                //-------------------------------------------------------
                //ext
                OnWriteClassStarted(state, contextCode, contextClassName, contextParentClassName, stateClassName);
        ...
        public static Dictionary<string, string> FeederInstance { get; private set; }

        //private Door door;
        private void WriteFields(CodeTypeDeclaration contextCode)
        {
            if (FeederInstance == null) FeederInstance = new Dictionary<string, string>();

            foreach (ObjectType obj in Model.settings.@object)
            {
                string fName = GetObjectFieldName(obj.instance);
                string className = obj.@class;
                CodeMemberField objectField = new CodeMemberField(className, fName);

                //keep data of the class instance name
                if (!FeederInstance.ContainsKey(className))
                    FeederInstance.Add(className, fName);

                objectField.Attributes = MemberAttributes.Private;
                contextCode.Members.Add(objectField);

                objectField.Comments.Add(new CodeCommentStatement("Field:" + fName));
        ...
        partial void OnWriteEventsStarting(CodeTypeDeclaration contextCode, StateType state);
        ...
        {
            //ext
            OnWriteEventsStarting(contextCode, state);
            ...
            {
                //ext
                OnWriteEventsIteratingEventStarting(contextCode, state, evt);
        ...
        /// <summary>
        /// Gets executed if th emodel is ASync
        /// </summary>
        /// <param name="contextCode"></param>
        ...
        private void WriteEventAsync(CodeTypeDeclaration contextCode, StateType state, EventType evt, int evenId)
        {
            ...
            //ext
            OnWriteEventAsyncStarting(contextCode, state, evt, evenId);
        ...
        //}
         ...
         //todo Use :   GetPropertyName ( ObjectType );
        //               GetObjectFieldName ( ObjectType );
        public string GetInstanceId(string className)
        {
       
            if (FeederInstance.ContainsKey(className))
                return FeederInstance[className].ToSaveName();
            return "";
        }
        ...
        /// <summary>
        /// when the Write event sync is entered and none has been done
        /// </summary>
        /// <param name="contextCode"></param>
        /// <param name="state"></param>
        /// <param name="evt"></param>
 ...
 OnWriteEventSyncEntered
            ...
            //ext
            OnWriteEventSyncStarting(contextCode, state, evt, eventMethod, Model.settings.asynchronous,methodInvoke);
            ...
            //Doing it in the Partial to have the call in the right position
